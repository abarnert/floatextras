...
def from_tuple(t, *, direct=False):
    ...
    try:
        ...
        digits = _int_to_bits(digits, sys.float_info.mant_digits-1)
...
def float_difference(f, g, *, direct=False):
    ...
    bits--loosely, in ulps (Units of Least Precision).

    This follows the same rules as next_plus, so for any f except
    +inf and nan, float_difference(f, next_plus(f)) will be -1.
     
    For two numbers with the same sign and exponent, the difference
    in ulps is unambiguous, and the same as the difference in bits.
...
def make_nan(payload, quiet=True, sign=0):
    """make_nan(payload, quiet, sign) -> NaN"""
    if not payload and not quiet:
        raise ValueError('A signaling NaN cannot have payload 0')
    digits = (quiet,) + _int_to_bits(payload, sys.float_info.mant_dig-2)
    return from_tuple(sign, sys.max_exp, digits)
